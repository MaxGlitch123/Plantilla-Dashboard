...
const EmployeeModal: React.FC<EmployeeModalProps> = ({ isOpen, onClose, onSave, employee, roles }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validación de roles - asegurarse que al menos se seleccionó un rol
    if (formData.roles.length === 0) {
      alert('Debes seleccionar al menos un rol para el empleado');
      return;
    }
    
    setPasswordError('');
    console.log('Enviando formulario con roles:', formData.roles);
    onSave(formData);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-lg mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          />
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Rol (selecciona solo uno)</label>
