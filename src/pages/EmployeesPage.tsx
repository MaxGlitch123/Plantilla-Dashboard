...
const EmployeesPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        console.log('Iniciando carga de empleados y roles...');
        
        // Cargar roles primero para poder diagnosticar problemas
        try {
          console.log('Intentando cargar roles...');
          const rolesData = await fetchRoles();
          console.log('Roles cargados correctamente:', rolesData);
          setRoles(rolesData.filter(r => !r.deleted));
        } catch (roleErr) {
          console.error('Error específico al cargar roles:', roleErr);
        }
        ...
        console.error('Error general al cargar empleados o roles:', err);
      } finally {
        setLoading(false);
      }
    };
